...
public sealed record McpServiceError(string Code, string Message, int? BytesWritten = null);
public sealed record McpServiceResult<T>(T? Value, McpServiceError? Error)
{
	...
	public static McpServiceResult<T> Failure(string code, string message, int? bytesWritten = null) => new(default, new(code, message, bytesWritten));
}
